...
func sword_strike_state_setter() -> void:
	if sword_striking:
		is_walking = false
		is_sliding = false
		is_jumping = false
...
func handle_falling() -> void:
	if !is_on_floor() && !is_on_wall() && !is_on_ceiling() && velocity.y > 0 && !double_jumping:
		is_falling = true
	if is_falling:
		is_walking = false
		is_sliding = false
...
func walking_state_setter() -> void:
	if is_walking:
		is_falling = false
		is_sliding = false
		is_jumping = false
...
func double_jumping_state_setter ()->void:
	if double_jumping:
		is_walking = false
		is_falling = false
		is_sliding = false
...
var bullet_path = preload("res://assets/Scenes/weapons/Bullet.tscn")
...
func handle_wall_slide(GRAVITY: Vector2, delta: float) -> void:
	if is_on_wall() and not is_on_floor() and (velocity.y > 0 or velocity.y < 0) and not damaged and not sword_striking:
...
func disable_hit () -> void:
	$coin_collector/CollisionShape2D.disabled = true
	$CollisionShape2DNormal.disabled = true
...
func _physics_process(delta: float) -> void:
	...
	if last_direction < 0:
		$SwordCollider.position.x = -1 * position_sword
	else:
		$SwordCollider.position.x = position_sword
	#DebugWallSlide=================================================================================
	if is_sliding:
	...
	if is_on_floor():
		...
		double_jumping = false
	#StatesSet======================================================================================
	#double_jumping_state_setter()	
	#walking_state_setter()
	#handle_falling()
	#slide_state_setter()
	#sword_strike_state_setter()
	#===============================================================================================
	...
	#Inventory Handling==============================================================================
	inventory_switcher()
	if Input.is_action_just_pressed("fire") and damaged == false:
		if current_weapon_index == 0 and has_gun:
			shoot(last_direction, bullet_spawn_point)
		elif current_weapon_index == 1 and has_sword and is_sliding==false:
			sword_slash()
			#disable_hit()
	...
	if is_on_floor_only() or velocity.y < 0 or sword_striking==false:
		is_sliding = false
	...
	if not is_on_floor() and is_on_wall() and not damaged and not sword_striking:
