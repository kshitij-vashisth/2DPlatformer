...
@onready var falling_panel_2: Panel = $"../FallingPanel2"
...
func _on_button_pressed() -> void:
	...
	falling_panel_2.show()
	Anima.begin_single_shot($"../FallingPanel2") \
	.then(Anima.Node($"../FallingPanel2").anima_scale_y(1.0, 0.3).anima_from(0)) \
	.then(Anima.Node($"../FallingPanel2/VBoxContainer/RichTextLabel").anima_animation('typewrite', 0.03) ) \
	.then( Anima.Node($"../FallingPanel2/Button").anima_animation('tada', 0.5 ).anima_delay(-0.2) ) \
	.then( Anima.Node($"../FallingPanel2").anima_animation('tada', 0.5 ).anima_delay(-0.2) ) \
	.set_visibility_strategy(ANIMA.VISIBILITY.TRANSPARENT_ONLY) \
	.play_with_delay(0.5)
